...
KinectSensor::KinectSensor()
    : m_elevationAngle( 0 ),
    m_hasElevationAngleChanged( false )
{
...
HRESULT KinectSensor::Init(NUI_IMAGE_TYPE depthType, NUI_IMAGE_RESOLUTION depthRes, BOOL bNearMode, BOOL bFallbackToDefault, NUI_IMAGE_TYPE colorType, NUI_IMAGE_RESOLUTION colorRes, BOOL bSeatedSkeletonMode)
{
	...
	hr = m_VideoBuffer->Allocate( width, height, VIDEO_BUFFER_FORMAT );
	if (FAILED(hr))
	{
		return hr;
	}
	...
	hr = m_DepthBuffer->Allocate( width, height, DEPTH_BUFFER_FORMAT );
	if (FAILED(hr))
	{
		return hr;
	}
	...
	if( FAILED(hr) )
	{
     ...
     NuiCameraElevationGetAngle( &m_elevationAngle );
...
DWORD WINAPI KinectSensor::ProcessThread(LPVOID pParam)
{
	...
	while (true)
	{
        ...
        // tilt kinect camera
        if( pthis->m_hasElevationAngleChanged )
        {
            HRESULT hr = NuiCameraElevationSetAngle( pthis->m_elevationAngle );
            if( S_OK == hr )
            {
                pthis->m_hasElevationAngleChanged = false;
            }
        }
	}
...
LONG KinectSensor::getElevationAngle() const
{
    return m_elevationAngle;
}

void KinectSensor::setElevationAngle(LONG elevationAngle)
{
    if( elevationAngle != m_elevationAngle )
    {
        m_elevationAngle = elevationAngle;
        m_hasElevationAngleChanged = true;
    }
}
